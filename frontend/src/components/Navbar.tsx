    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 w-screen py-6 px-2 sm:px-4  border-black items-center fixed top-0 filter backdrop-filter backdrop-blur-lg rounded-b-md shadow-lg">
        ...
        <Link
          ...
          className="font-poppins text-2xl border-2 border-black rounded-full py-2 sm:px-6 px-4 font-semibold m-auto shadow-stone-300 shadow-lg hover:bg-black hover:text-white invisible w-0 md:w-28 md:visible absolute sm:static "
        >
          post
        </Link>
        {pathname === "/profile" ? (
          ...
          >
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="size-8 invisible w-0 sm:w-8 sm:visible"
            >
              <path
                strokeLinecap="round"
            ...
            <div className="text-xl truncate">{currentUserVal?.name}</div>
          </Link>
        ) : (
          <button
